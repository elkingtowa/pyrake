...
from pyrake.core.engine import ExecutionEngine
from pyrake.resolver import CachingThreadedResolver
from pyrake.extension import ExtensionManager
from pyrake.signalmanager import SignalManager
from pyrake.utils.ossignal import install_shutdown_handlers, signal_names
from pyrake.utils.misc import load_object
...
class Crawler(object):
    ...
    def install(self):
        # TODO: remove together with pyrake.project.crawler usage
        import pyrake.project
        assert not hasattr(pyrake.project, 'crawler'), "crawler already installed"
        pyrake.project.crawler = self

    def uninstall(self):
        # TODO: remove together with pyrake.project.crawler usage
        import pyrake.project
        assert hasattr(pyrake.project, 'crawler'), "crawler not installed"
        del pyrake.project.crawler
...
class CrawlerProcess(object):
    """ A class to run multiple pyrake crawlers in a process sequentially"""
    ...
    # any moment.
    #
    # start_crawling and start_reactor are called from pyrake.commands.shell
    # They are splitted because reactor is started on a different thread than IPython shell.
    #
    def start_crawling(self):
        log.pyrake_info(self.settings)
