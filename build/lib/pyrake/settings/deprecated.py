...
DEPRECATED_SETTINGS = [
    ...
    ('BOT_VERSION', 'no longer used (user agent defaults to pyrake now)'),
    ...
    ('SELECTORS_BACKEND', 'use pyrake_SELECTORS_BACKEND environment variable instead'),
...
def check_deprecated_settings(settings):
    ...
    if deprecated:
        ...
        msg += " (ask pyrake-users@googlegroups.com for alternatives):"
        msg = msg + "\n    " + "\n    ".join("%s: %s" % x for x in deprecated)
        warnings.warn(msg, pyrakeDeprecationWarning)
