...
import pyrake
from pyrake.utils.python import unicode_to_str
from pyrake.settings import overridden_settings
...
class pyrakeFileLogObserver(log.FileLogObserver):
...
def _adapt_eventdict(eventDict, log_level=INFO, encoding='utf-8', prepend_level=True):
    """Adapt Twisted log eventDict making it suitable for logging with a pyrake
    log observer. It may return None to indicate that the event should be
    ignored by a pyrake log observer.
    ...
    # ignore non-error messages from outside pyrake
    if ev.get('system') != 'pyrake' and not ev['isError']:
        return
...
def start(logfile=None, loglevel='INFO', logstdout=True, logencoding='utf-8', crawler=None):
    ...
    log_observer = pyrakeFileLogObserver(file, loglevel, logencoding, crawler)
...
def pyrake_info(settings):
    log_observer = start_from_settings(settings)
    if log_observer:
        msg("pyrake %s started (bot: %s)" % (pyrake.__version__,
            settings['BOT_NAME']))

        msg("Optional features available: %s" % ", ".join(pyrake.optional_features),
            level=INFO)
