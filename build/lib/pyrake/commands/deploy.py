...
from pyrake.utils.http import basic_auth_header
from pyrake.utils.python import retry_on_eintr
from pyrake.utils.conf import get_config, closest_pyrake_cfg

_SETUP_PY_TEMPLATE = \
"""# Automatically created by: pyrake deploy
...
setup(
    ...
    entry_points = {'pyrake': ['settings = %(settings)s']},
)
"""
...
class Command(pyrakeCommand):
    ...
    def short_desc(self):
        return "Deploy project in pyraked target"

    def long_desc(self):
        return "Deploy the current project into the given pyraked server " \
            "(known as target)"
...
def _build_egg():
    ...
    d = tempfile.mkdtemp(prefix="pyrakedeploy-")
