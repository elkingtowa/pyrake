...
How does pyrake compare to BeautifulSoup or lxml?
-------------------------------------------------

`BeautifulSoup`_ and `lxml`_ are libraries for parsing HTML and XML. pyrake is
an application framework for writing web spiders that crawl web sites and
extract data from them.

pyrake provides a built-in mechanism for extracting data (called
...
In other words, comparing `BeautifulSoup`_ (or `lxml`_) to pyrake is like
...
What Python versions does pyrake support?
-----------------------------------------

pyrake is supported under Python 2.7 only.
Python 2.6 support was dropped starting at pyrake 0.20.

Does pyrake work with Python 3?
---------------------------------

No, but there are plans to support Python 3.3+.
At the moment, pyrake works with Python 2.7.

.. seealso:: :ref:`faq-python-versions`.

Did pyrake "steal" X from Django?
---------------------------------
...
We'd be proud if pyrake serves as an inspiration for other projects. Feel free
to steal from us!

.. _Django: http://www.djangoproject.com

Does pyrake work with HTTP proxies?
-----------------------------------

Yes. Support for HTTP proxies is provided (since pyrake 0.8) through the HTTP
Proxy downloader middleware. See
:class:`~pyrake.contrib.downloadermiddleware.httpproxy.HttpProxyMiddleware`.
...
pyrake crashes with: ImportError: No module named win32api
----------------------------------------------------------
...
Does pyrake crawl in breadth-first or depth-first order?
--------------------------------------------------------

By default, pyrake uses a `LIFO`_ queue for storing pending requests, which
...
setting the following settings::
    ...
    DEPTH_PRIORITY = 1
    SCHEDULER_DISK_QUEUE = 'pyrake.squeue.PickleFifoDiskQueue'
    SCHEDULER_MEMORY_QUEUE = 'pyrake.squeue.FifoMemoryQueue'

My pyrake crawler has memory leaks. What can I do?
--------------------------------------------------
...
How can I make pyrake consume less memory?
------------------------------------------
...
Yes, see :class:`~pyrake.contrib.downloadermiddleware.httpauth.HttpAuthMiddleware`.

Why does pyrake download pages in English instead of my native language?
------------------------------------------------------------------------
...
Where can I find some example pyrake projects?
----------------------------------------------
...
For more info see:
:class:`~pyrake.contrib.spidermiddleware.offsite.OffsiteMiddleware`.

What is the recommended way to deploy a pyrake crawler in production?
---------------------------------------------------------------------

See :ref:`topics-pyraked`.

Can I use JSON for large exports?
---------------------------------

It'll depend on how large your output is. See :ref:`this warning
<json-with-large-data>` in :class:`~pyrake.contrib.exporter.JsonItemExporter`
documentation.
...
Yes, but you can also use the pyrake shell which allows you too quickly analyze
...
To dump into a JSON file::
    ...
    pyrake crawl myspider -o items.json

To dump into a CSV file::

    pyrake crawl myspider -o items.csv

To dump into a XML file::

    pyrake crawl myspider -o items.xml
...
the functions ``xmliter`` and ``csviter`` from ``pyrake.utils.iterators``
module. In fact, this is what the feed spiders (see :ref:`topics-spiders`) use
under the cover.

Does pyrake manage cookies automatically?
-----------------------------------------

Yes, pyrake receives and keeps track of cookies sent by servers, and sends them
...
How can I see the cookies being sent and received from pyrake?
--------------------------------------------------------------
...
Raise the :exc:`~pyrake.exceptions.CloseSpider` exception from a callback. For
more info see: :exc:`~pyrake.exceptions.CloseSpider`.

How can I prevent my pyrake bot from getting banned?
----------------------------------------------------
...
This is caused by pyrake changes due to the singletons removal. The error is
most likely raised by a module (extension, middleware, pipeline or spider) in
your pyrake project that imports ``crawler`` from ``pyrake.project``. For
example::
...
pyrake command line tool has some backwards compatibility in place to support
the old import mechanism (with a deprecation warning), but this mechanism may
not work if you use pyrake differently (for example, as a library).
