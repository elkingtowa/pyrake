...
You can install pyrake using pip (which is the canonical way to install Python
packages).
...
**Don't** use the ``python-pyrake`` package provided by Ubuntu, they are
typically too old and slow to catch up with latest pyrake.
...
You can follow the generic instructions or install pyrake from `AUR pyrake package`::
...
.. _AUR pyrake package: https://aur.archlinux.org/packages/pyrake/
