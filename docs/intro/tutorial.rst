...
In this tutorial, we'll assume that pyrake is already installed on your system.
...
1. Creating a new pyrake project
...
pyrake is written in Python_. If you're new to the language you might want to
start by getting an idea of what the language is like, to get the most out of
pyrake.  If you're already familiar with other languages, and want to learn
...
Before you start scraping, you will have set up a new pyrake project. Enter a
directory where you'd like to store your code and then run::

    pyrake startproject tutorial
...
* ``pyrake.cfg``: the project configuration file
...
They are declared by creating a :class:`pyrake.Item <pyrake.item.Item>` class and defining
its attributes as :class:`pyrake.Field <pyrake.item.Field>` objects, like you will in an ORM
...
To create a Spider, you must subclass :class:`pyrake.Spider <pyrake.spider.Spider>` and
define the three main mandatory attributes:

* :attr:`~pyrake.spider.Spider.name`: identifies the Spider. It must be
  unique, that is, you can't set the same name for different Spiders.

* :attr:`~pyrake.spider.Spider.start_urls`: is a list of URLs where the
  Spider will begin to crawl from.  So, the first pages downloaded will be those
  listed here. The subsequent URLs will be generated successively from data
  contained in the start URLs.

* :meth:`~pyrake.spider.Spider.parse` is a method of the spider, which will
  be called with the downloaded :class:`~pyrake.http.Response` object of each
  ...
  The :meth:`~pyrake.spider.Spider.parse` method is in charge of processing
  the response and returning scraped data (as :class:`~pyrake.item.Item`
  objects) and more URLs to follow (as :class:`~pyrake.http.Request` objects).
...
``dmoz_spider.py`` under the ``tutorial/spiders`` directory::
    ...
    class DmozSpider(pyrake.Spider):
        name = "dmoz"
...
will get an output similar to this::
    ...
    2014-01-23 18:13:07-0400 [pyrake] INFO: pyrake started (bot: tutorial)
    2014-01-23 18:13:07-0400 [pyrake] INFO: Optional features available: ...
    2014-01-23 18:13:07-0400 [pyrake] INFO: Overridden settings: {}
    2014-01-23 18:13:07-0400 [pyrake] INFO: Enabled extensions: ...
    2014-01-23 18:13:07-0400 [pyrake] INFO: Enabled downloader middlewares: ...
    2014-01-23 18:13:07-0400 [pyrake] INFO: Enabled spider middlewares: ...
    2014-01-23 18:13:07-0400 [pyrake] INFO: Enabled item pipelines: ...
...
pyrake creates :class:`pyrake.Request <pyrake.http.Request>` objects
...
These Requests are scheduled, then executed, and :class:`pyrake.http.Response`
objects are returned and then fed back to the spider, through the
:meth:`~pyrake.spider.Spider.parse` method.
...
There are several ways to extract data from web pages. pyrake uses a mechanism
based on `XPath`_ or `CSS`_ expressions called :ref:`pyrake Selectors
...
For working with XPaths, pyrake provides :class:`~pyrake.selector.Selector`
...
* :meth:`~pyrake.selector.Selector.xpath`: returns a list of selectors, each of
  them representing the nodes selected by the xpath expression given as
  argument.

* :meth:`~pyrake.selector.Selector.css`: returns a list of selectors, each of
  them representing the nodes selected by the CSS expression given as argument.

* :meth:`~pyrake.selector.Selector.extract`: returns a unicode string with the
  selected data.

* :meth:`~pyrake.selector.Selector.re`: returns a list of unicode strings
...
To illustrate the use of Selectors we're going to use the built-in :ref:`pyrake
shell <topics-shell>`, which also requires IPython (an extended Python console)
installed on your system.

To start a shell, you must go to the project's top level directory and run::

    pyrake shell "http://www.dmoz.org/Computers/Programming/Languages/Python/Books/"

.. note::

   Remember to always enclose urls with quotes when running pyrake shell from
...
This is what the shell looks like::
    ...
    [s]   crawler    <pyrake.crawler.Crawler object at 0x3636b50>
    [s]   item       {}
    [s]   request    <GET http://www.dmoz.org/Computers/Programming/Languages/Python/Books/>
    [s]   response   <200 http://www.dmoz.org/Computers/Programming/Languages/Python/Books/>
    [s]   settings   <pyrake.settings.Settings object at 0x3fadc50>
...
:class:`~pyrake.item.Item` objects are custom python dicts; you can access the
...
:class:`~pyrake.item.Item` objects. So, in order to return the data we've
...
.. note:: You can find a fully-functional variant of this spider in the dirbot_
   project available at https://github.com/pyrake/dirbot
...
<topics-feed-exports>`, with the following command::
    ...
    pyrake crawl dmoz -o items.json
...
This tutorial covers only the basics of pyrake, but there's a lot of other
