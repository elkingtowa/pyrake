...
pyrake is an application framework for crawling web sites and extracting
structured data which can be used for a wide range of useful applications, like
data mining, information processing or historical archival.

Even though pyrake was originally designed for `screen scraping`_ (more
...
The purpose of this document is to introduce you to the concepts behind pyrake
so you can get an idea of how it works and decide if pyrake is what you need.
...
provide any API or mechanism to access that info programmatically.  pyrake can
...
The first thing is to define the data we want to scrape. In pyrake, this is
done through :ref:`pyrake Items <topics-items>` (Torrent files, in this case).

This would be our Item::

    import pyrake

    class TorrentItem(pyrake.Item):
        url = pyrake.Field()
        name = pyrake.Field()
...
``scraped_data.json`` with the scraped data in JSON format::
    ...
    pyrake crawl mininova -o scraped_data.json
...
You've seen how to extract and store items from a website using pyrake, but
this is just the surface. pyrake provides a lot of powerful features for making
...
* Support for :ref:`extending pyrake <extending-pyrake>` by plugging
...
* A :ref:`System service <topics-pyraked>` designed to ease the deployment and
  run of your spiders in production.

* A :ref:`Telnet console <topics-telnetconsole>` for hooking into a Python
  console running inside your pyrake process, to introspect and debug your
  crawler
...
The next obvious steps are for you to `download pyrake`_, read :ref:`the
tutorial <intro-tutorial>` and join `the community`_. Thanks for your
interest!

.. _download pyrake: http://pyrake.org/download/
.. _the community: http://pyrake.org/community/
