...
- pem file is used by mockserver and required by pyrake bench (:commit:`5eddc68`)
- pyrake bench needs pyrake.tests* (:commit:`d6cb999`)
...
- There is a trove classifier for pyrake framework! (:commit:`4c701d7`)
...
- pyrake.bat is not needed anymore (:commit:`c3861cf`)
...
- fix tests to account for https://github.com/pyrake/w3lib/pull/23 (:commit:`f8d366a`)
...
- Improve pyrake top-level namespace (:issue:`494`, :issue:`684`)
...
- Expose current crawler in pyrake shell (:issue:`557`)
...
- Make pyrake.version_info a tuple of integers (:issue:`681`, :issue:`692`)
...
- Improve `pyrake check` contracts command (:issue:`733`, :issue:`752`)
...
- Sort spider list output of `pyrake list` command (:issue:`742`) 
...
- Expose current crawler in the pyrake shell. (:commit:`5349cec`)
...
- modify the version of pyrake ubuntu package (:commit:`725900d`)
...
- [**Backwards incompatible**] Switched HTTPCacheMiddleware backend to filesystem (:issue:`541`)
  To restore old backend set `HTTPCACHE_STORAGE` to `pyrake.contrib.httpcache.DbmCacheStorage`
- Proxy \https:// urls using CONNECT method (:issue:`392`, :issue:`397`)
- Add a middleware to crawl ajax crawleable pages as defined by google (:issue:`343`)
- Rename pyrake.spider.BaseSpider to pyrake.spider.Spider (:issue:`510`, :issue:`519`)
...
- pyrake.mail.MailSender now can connect over TLS or upgrade using STARTTLS (:issue:`327`)
...
- Travis-CI now tests pyrake changes against development versions of `w3lib` and `queuelib` python packages.
...
- Fix pyrake shell bugs (:issue:`418`, :issue:`407`)
...
- Backport `pyrake check` command fixes and backward compatible multi
...
- Bugfix `pyrake.utils.gz.gunzip` broken by changes in python 2.7.4 (:commit:`4dc76e`)
...
- Extend pyrake commands using setuptools entry points (:issue:`260`)
- Allow spider `allowed_domains` value to be set/tuple (:issue:`261`)
- Support `settings.getdict` (:issue:`269`)
- Simplify internal `pyrake.core.scraper` slot handling (:issue:`271`)
...
- Moved persistent (on disk) queues to a separate project (queuelib_) which pyrake now depends on
- Add pyrake commands using external libraries (:issue:`260`)
- Added ``--pdb`` option to ``pyrake`` command line tool
...
- log overridden settings when pyrake starts
...
- obey request method when pyrake deploy is redirected to a new endpoint (:commit:`8c4fcee`)
...
- fix bug in pyrake parse command when spider is not specified explicitly. closes #209 (:commit:`c72e682`)
...
- pyrake contracts: python2.6 compat (:commit:`a4a9199`)
- pyrake contracts verbose option (:commit:`ec41673`)
- proper unittest-like output for pyrake contracts (:commit:`86635e4`)
- added open_in_browser to debugging doc (:commit:`c9b690d`)
- removed reference to global pyrake stats from settings doc (:commit:`dd55067`)
...
- better backwards compatibility for pyrake.conf.settings (:commit:`3403089`)
- extended documentation on how to access crawler stats from extensions (:commit:`c4da0b5`)
- removed .hgtags (no longer needed now that pyrake uses git) (:commit:`d52c188`)
...
- added :meth:`~pyrake.contrib.spidermiddleware.SpiderMiddleware.process_start_requests` method to spider middlewares
...
- removed modules: ``pyrake.xlib.BeautifulSoup`` and ``pyrake.xlib.ClientForm``
...
- decoupled encoding detection code to `w3lib.encoding`_, and ported pyrake code to use that mdule
- dropped support for Python 2.5. See http://blog.scrapinghub.com/2012/02/27/pyrake-0-15-dropping-support-for-python-2-5/
- dropped support for Twisted 2.5
- added :setting:`REFERER_ENABLED` setting, to control referer middleware
- changed default user agent to: ``pyrake/VERSION (+http://pyrake.org)``
- removed (undocumented) ``HTMLImageLinkExtractor`` class from ``pyrake.contrib.linkextractors.image``
...
- replaced memory usage acounting with (more portable) `resource`_ module, removed ``pyrake.utils.memory`` module
- removed signal: ``pyrake.mail.mail_sent``
- removed ``TRACK_REFS`` setting, now :ref:`trackrefs <topics-leaks-trackrefs>` is always enabled
- DBM is now the default storage backend for HTTP cache middleware
- number of log messages (per level) are now tracked through pyrake stats (stat name: ``log_count/LEVEL``)
- number received responses are now tracked through pyrake stats (stat name: ``response_received_count``)
- removed ``pyrake.log.started`` attribute

0.14.4
------

- added precise to supported ubuntu distros (:commit:`b7e46df`)
- fixed bug in json-rpc webservice reported in https://groups.google.com/d/topic/pyrake-users/qgVBmFybNAQ/discussion. also removed no longer supported 'run' command from extras/pyrake-ws.py (:commit:`340fbdb`)
...
- removed some obsolete remaining code related to sqlite support in pyrake (:commit:`0665175`)
...
- pyraked: fixed documentation link (:commit:`2b4e4c3`)
...
- fixed formatting of pyraked doc (:commit:`8bf19e6`)
...
- added ``-o`` option to ``pyrake crawl``, a shortcut for dumping scraped items into a file (or standard output using ``-``)
- Added support for passing custom settings to pyraked ``schedule.json`` api (:rev:`2779`, :rev:`2783`)
...
- Moved spider queues to pyraked: `pyrake.spiderqueue` -> `pyraked.spiderqueue` (:rev:`2708`)
- Moved sqlite utils to pyraked: `pyrake.utils.sqlite` -> `pyraked.sqlite` (:rev:`2781`)
...
- Make handling of gzipped responses more robust (#319, :rev:`2643`). Now pyrake will try and decompress as much as possible from a gzipped response, instead of failing with an `IOError`.
- Simplified !MemoryDebugger extension to use stats for dumping memory debugging info (:rev:`2639`)
- Added new command to edit spiders: ``pyrake edit`` (:rev:`2636`) and `-e` flag to `genspider` command that uses it (:rev:`2653`)
...
- Stats are now dumped to pyrake log (default value of :setting:`STATS_DUMP` setting has been changed to `True`). This is to make pyrake users more aware of pyrake stats and the data that is collected there.
...
- Added ``listjobs.json`` API to pyraked (:rev:`2571`)
...
- Several improvements to pyraked and Link extractors
...
- Reduced pyrake codebase by striping part of pyrake code into two new libraries:
   - `w3lib`_ (several functions from ``pyrake.utils.{http,markup,multipart,response,url}``, done in :rev:`2584`)
   - `scrapely`_ (was ``pyrake.contrib.ibl``, done in :rev:`2586`)
- Removed unused function: `pyrake.utils.request.request_info()` (:rev:`2577`)
- Removed googledir project from `examples/googledir`. There's now a new example project called `dirbot` available on github: https://github.com/pyrake/dirbot
- Removed support for default field values in pyrake items (:rev:`2616`)
...
- Removed support for passing urls to ``pyrake crawl`` command (use ``pyrake parse`` instead) (:rev:`2704`)
- Removed deprecated Execution Queue (:rev:`2704`)
- Removed (undocumented) spider context extension (from pyrake.contrib.spidercontext) (:rev:`2780`)
- removed ``CONCURRENT_SPIDERS`` setting (use pyraked maxproc instead) (:rev:`2789`)
...
- Added verbose option to ``pyrake version`` command, useful for bug reports (#298)
- HTTP cache now stored by default in the project data dir (#279)
- Added project data storage directory (#276, #277)
- Documented file structure of pyrake projects (see command-line tool doc)
- New lxml backend for XPath selectors (#147)
- Per-spider settings (#245)
- Support exit codes to signal errors in pyrake commands (#248)
- Added ``-c`` argument to ``pyrake shell`` command
...
- pyraked now uses one process per spider
- It stores one log file per spider run, and rotate them keeping the lastest 5 logs per spider (by default)
- A minimal web ui was added, available at http://localhost:6800 by default
- There is now a `pyrake server` command to start a pyraked server of the current project
...
- Deprecated ``runserver`` command in favor of ``server`` command which starts a pyraked server. See also: pyraked changes
- Deprecated ``queue`` command in favor of using pyraked ``schedule.json`` API. See also: pyraked changes
...
- New pyrake service called ``pyraked`` for deploying pyrake crawlers in production (#218) (documentation available)
- Simplified Images pipeline usage which doesn't require subclassing your own images pipeline now (#217)
- pyrake shell now shows the pyrake log by default (#206)
- Refactored execution queue in a common base code and pluggable backends called "spider queues" (#220)
- New persistent spider queue (based on SQLite) (#198), available by default, which allows to start pyrake in server mode and then schedule spiders to run.
- Added documentation for pyrake command-line tool and all its available sub-commands. (documentation available)
...
- New `pyrake` command which replaces the old `pyrake-ctl.py` (#199)
  - there is only one global `pyrake` command now, instead of one `pyrake-ctl.py` per project
  - Added `pyrake.bat` script for running more conveniently from Windows
...
- Removed ``UrlFilterMiddleware`` from ``pyrake.contrib`` (already disabled by default)
...
- Changes to pyrake Manager (now called "Crawler"):
   - ``pyrake.core.manager.pyrakeManager`` class renamed to ``pyrake.crawler.Crawler``
   - ``pyrake.core.manager.pyrakemanager`` singleton moved to ``pyrake.project.crawler``
- Moved module: ``pyrake.contrib.spidermanager`` to ``pyrake.spidermanager``
- Spider Manager singleton moved from ``pyrake.spider.spiders`` to the ``spiders` attribute of ``pyrake.project.crawler`` singleton.
- moved Stats Collector classes: (#204)
   - ``pyrake.stats.collector.StatsCollector`` to ``pyrake.statscol.StatsCollector``
   - ``pyrake.stats.collector.SimpledbStatsCollector`` to ``pyrake.contrib.statscol.SimpledbStatsCollector``
...
- moved ``pyrake.core.signals`` module to ``pyrake.signals``
   - backwards compatibility kept (with deprecation warning)
- moved ``pyrake.core.exceptions`` module to ``pyrake.exceptions``
   - backwards compatibility kept (with deprecation warning)
- added ``handles_request()`` class method to ``BaseSpider``
- dropped ``pyrake.log.exc()`` function (use ``pyrake.log.err()`` instead)
- dropped ``component`` argument of ``pyrake.log.msg()`` function
- dropped ``pyrake.log.log_level`` attribute
...
- Added SMTP-AUTH support to pyrake.mail
- New settings added: ``MAIL_USER``, ``MAIL_PASS`` (:rev:`2065` | #149)
- Added new pyrake-ctl view command - To view URL in the browser, as seen by pyrake (:rev:`2039`)
- Added web service for controlling pyrake process (this also deprecates the web console. (:rev:`2053` | #167)
- Support for running pyrake as a service, for production systems (:rev:`1988`, :rev:`2054`, :rev:`2055`, :rev:`2056`, :rev:`2057` | #168)
...
- ``pyrake.command`` modules relocation (:rev:`2035`, :rev:`2036`, :rev:`2037`)
...
- Moved module: ``pyrake.management.telnet`` to ``pyrake.telnet`` (:rev:`2047`)
...
- Changed ``pyrake.utils.response.get_meta_refresh()`` signature (:rev:`1804`)
- Removed deprecated ``pyrake.item.ScrapedItem`` class - use ``pyrake.item.Item instead`` (:rev:`1838`)
- Removed deprecated ``pyrake.xpath`` module - use ``pyrake.selector`` instead. (:rev:`1836`)
...
- Changed Item pipeline to use spiders instead of domains
   ...
   - To quickly port your code (to work with pyrake 0.8) just use ``spider.domain_name`` where you previously used ``domain``.
- Changed Stats API to use spiders instead of domains (:rev:`1849` | #113)
   ...
   - To quickly port your code (to work with pyrake 0.8) just use ``spider.domain_name`` where you previously used ``domain``. ``spider_stats`` contains exactly the same data as ``domain_stats``.
- ``CloseDomain`` extension moved to ``pyrake.contrib.closespider.CloseSpider`` (:rev:`1833`)
   - Its settings were also renamed:
      - ``CLOSEDOMAIN_TIMEOUT`` to ``CLOSESPIDER_TIMEOUT``
      - ``CLOSEDOMAIN_ITEMCOUNT`` to ``CLOSESPIDER_ITEMCOUNT``
- Removed deprecated ``pyrakeSETTINGS_MODULE`` environment variable - use ``pyrake_SETTINGS_MODULE`` instead (:rev:`1840`)
...
- Removed obsolete ``pyrake.utils.markup.remove_escape_chars`` function - use ``pyrake.utils.markup.replace_escape_chars`` instead (:rev:`1865`)

0.7
---

First release of pyrake.
...
.. _w3lib: https://github.com/pyrake/w3lib
.. _scrapely: https://github.com/pyrake/scrapely
.. _marshal: http://docs.python.org/library/marshal.html
.. _w3lib.encoding: https://github.com/pyrake/w3lib/blob/master/w3lib/encoding.py
.. _lxml: http://lxml.de/
.. _ClientForm: http://wwwsearch.sourceforge.net/old/ClientForm/
.. _resource: http://docs.python.org/library/resource.html
.. _queuelib: https://github.com/pyrake/queuelib
