...
Consider the following pyrake spider below::
    ...
    class MySpider(pyrake.Spider):
        ...
        def parse(self, response):
            ...
            for item_url in item_urls:
                yield pyrake.Request(item_url, self.parse_item)

        def parse_item(self, response):
            item = MyItem()
            # populate `item` fields
            # and extract item_details_url
            yield pyrake.Request(item_details_url, self.parse_details, meta={'item': item})
...
use the ``meta`` functionality of :class:`~pyrake.http.Request` to pass a
...
In order to see the item scraped from a specific url::
    ...
    $ pyrake parse --spider=myspider -c parse_item -d 2 <item_url>
...
Using the ``--verbose`` or ``-v`` option we can see the status at each depth level::
    ...
    $ pyrake parse --spider=myspider -c parse_item -d 2 -v <item_url>
    [ ... pyrake log lines crawling example.com spider ... ]
...
using::
    ...
    $ pyrake parse --spider=myspider -d 3 'http://example.com/page1'
...
``open_in_browser`` will open a browser with the response received by pyrake at
