...
sources, typically, web pages. pyrake provides the :class:`Item` class for this
purpose.
...
objects. Here is an example::
    ...
    class Product(pyrake.Item):
        ...
        price = pyrake.Field()
        stock = pyrake.Field()
        last_updated = pyrake.Field(serializer=str)

.. note:: Those familiar with `Django`_ will notice that pyrake Items are
   declared similar to `Django Models`_, except that pyrake Items are much
...
For example::
    ...
    class DiscountedProduct(Product):
        discount_percent = pyrake.Field(serializer=str)
        discount_expiration_date = pyrake.Field()
...
appending more values, or changing existing values, like this::
    ...
    class SpecificProduct(Product):
        name = pyrake.Field(Product.fields['name'], serializer=my_serializer)
