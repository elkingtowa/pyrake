...
1. You start by generating the initial Requests to crawl the first URLs, and
   ...
   :meth:`~pyrake.spider.Spider.start_requests` method which (by default)
   generates :class:`~pyrake.http.Request` for the URLs specified in the
   :attr:`~pyrake.spider.Spider.start_urls` and the
   :attr:`~pyrake.spider.Spider.parse` method as callback function for the
   Requests.

2. In the callback function, you parse the response (web page) and return either
   :class:`~pyrake.item.Item` objects, :class:`~pyrake.http.Request` objects,
   or an iterable of both. Those Requests will also contain a callback (maybe
   the same) and will then be downloaded by pyrake and then their
...
different kinds of default spiders bundled into pyrake for different purposes.
...
``-a`` option. For example::
    ...
    pyrake crawl myspider -a category=electronics
...
Spider arguments can also be passed through the pyraked ``schedule.json`` API.
See `pyraked documentation`_.
...
pyrake comes with some useful generic spiders that you can use, to subclass
...
with a ``TestItem`` declared in a ``myproject.items`` module::
    ...
    class TestItem(pyrake.Item):
        id = pyrake.Field()
        name = pyrake.Field()
        description = pyrake.Field()


.. module:: pyrake.spider
...
.. class:: Spider()
   ...
   must inherit from (either the ones that come bundled with pyrake, or the ones
   ...
   .. attribute:: name
       ...
       the spider is located (and instantiated) by pyrake, so it must be
   ...
   .. attribute:: allowed_domains
       ...
       :class:`~pyrake.contrib.spidermiddleware.offsite.OffsiteMiddleware` is enabled.
   ...
   .. method:: start_requests()
       ...
       This is the method called by pyrake when the spider is opened for
       ...
       the Requests. This method is also called only once from pyrake, so it's
       ...
       a POST request, you could do::
           ...
           def start_requests(self):
               return [pyrake.FormRequest("http://www.example.com/login",
   ...
   .. method:: make_requests_from_url(url)
       ...
       A method that receives a URL and returns a :class:`~pyrake.http.Request`
       object (or a list of :class:`~pyrake.http.Request` objects) to scrape. This
       ...
       (see :class:`~pyrake.http.Request` class for more info).

   .. method:: parse(response)

       This is the default callback used by pyrake to process downloaded
       ...
       iterable of :class:`~pyrake.http.Request` and/or
       :class:`~pyrake.item.Item` objects.

       :param response: the response to parse
       :type response: :class:~pyrake.http.Response`

   .. method:: log(message, [level, component])

       Log a message using the :func:`pyrake.log.msg` function, automatically
    ...
    class MySpider(pyrake.Spider):
        ...
        def parse(self, response):
            ...
            for url in response.xpath('//a/@href').extract():
                yield pyrake.Request(url, callback=self.parse)

.. module:: pyrake.contrib.spiders
...
.. class:: CrawlSpider
   ...
   .. method:: parse_start_url(response)
      ...
      :class:`~pyrake.item.Item` object, a :class:`~pyrake.http.Request`
...
.. class:: Rule(link_extractor, callback=None, cb_kwargs=None, follow=None, process_links=None, process_request=None)
   ...
   argument and must return a list containing :class:`~pyrake.item.Item` and/or
   :class:`~pyrake.http.Request` objects (or any subclass of them).
...
Let's now take a look at an example CrawlSpider with rules::
    ...
    import pyrake
    from pyrake.contrib.spiders import CrawlSpider, Rule
...
a :class:`~pyrake.item.Item` will be filled with it.
...
.. class:: XMLFeedSpider
    ...
    .. attribute:: iterator
        ...
        A string which defines the iterator to use. It can be either:
           ...
           - ``'html'`` - an iterator which uses :class:`~pyrake.selector.Selector`.
             Keep in mind this uses DOM parsing and must load all DOM in memory
             which could be a problem for big feeds

           - ``'xml'`` - an iterator which uses :class:`~pyrake.selector.Selector`.
             Keep in mind this uses DOM parsing and must load all DOM in memory
             which could be a problem for big feeds
    ...
    .. attribute:: namespaces
        ...
        :meth:`~pyrake.selector.Selector.register_namespace` method.
    ...
    .. method:: parse_node(response, selector)
        ...
        :class:`~pyrake.selector.Selector` for each node.  Overriding this
        method is mandatory. Otherwise, you spider won't work.  This method
        must return either a :class:`~pyrake.item.Item` object, a
        :class:`~pyrake.http.Request` object, or an iterable containing any of
        them.
...
These spiders are pretty easy to use, let's have a look at one example::
    ...
    from pyrake.contrib.spiders import XMLFeedSpider
...
prints them out, and stores some random data in an :class:`~pyrake.item.Item`.
...
:class:`CSVFeedSpider`::
    ...
    from pyrake import log
    from pyrake.contrib.spiders import CSVFeedSpider
    from myproject.items import TestItem
...
``parse`` callback::
    ...
    from pyrake.contrib.spiders import SitemapSpider
...
Combine SitemapSpider with other sources of urls::
    ...
    class MySpider(SitemapSpider):
        ...
        def start_requests(self):
            ...
            requests += [pyrake.Request(x, self.parse_other) for x in self.other_urls]
            return requests
...
.. _pyraked documentation: http://pyraked.readthedocs.org/
