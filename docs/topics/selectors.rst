...
pyrake comes with its own mechanism for extracting data. They're called
...
pyrake selectors are built over the `lxml`_ library, which means they're very
...
pyrake selectors are instances of :class:`~pyrake.selector.Selector` class
constructed by passing **text** or :class:`~pyrake.http.TextResponse`
object. It automatically chooses the best parsing rules (XML vs HTML) based on
input type::

    >>> from pyrake.selector import Selector
    >>> from pyrake.http import HtmlResponse
...
To explain how to use the selectors we'll use the `pyrake shell` (which
provides interactive testing) and an example page located in the pyrake
documentation server:

    http://doc.pyrake.org/en/latest/_static/selectors-sample1.html
...
First, let's open the shell::
    ...
    pyrake shell http://doc.pyrake.org/en/latest/_static/selectors-sample1.html
...
:class:`~pyrake.selector.SelectorList` instance, which is a list of new
...
:class:`~pyrake.selector.Selector` also have a ``.re()`` method for extracting
...
Being built atop `lxml`_, pyrake selectors also support some `EXSLT`_ extensions
...
with pyrake selectors, based on `this post from ScrapingHub's blog`_.
...
As it turns out, pyrake selectors allow you to chain selectors, so most of the time
...
.. class:: Selector(response=None, text=None, type=None)
  ...
  ``response`` is a :class:`~pyrake.http.HtmlResponse` or
  :class:`~pyrake.http.XmlResponse` object that will be used for selecting and
  extracting data.
  ...
  ``type`` defines the selector type, it can be ``"html"``, ``"xml"`` or ``None`` (default).
    ...
    inferred from the response type as follow:
        ...
        * ``"html"`` for :class:`~pyrake.http.HtmlResponse` type
        * ``"xml"`` for :class:`~pyrake.http.XmlResponse` type
...
a :class:`~pyrake.http.HtmlResponse` object like this::
...
:class:`~pyrake.http.XmlResponse` object like this::
...
First, we open the shell with the url we want to scrape::
    ...
    $ pyrake shell https://github.com/blog.atom
