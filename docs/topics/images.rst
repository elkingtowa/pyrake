...
.. currentmodule:: pyrake.contrib.pipeline.images

pyrake provides an :doc:`item pipeline </topics/item-pipeline>` for downloading
...
3. When the item reaches the :class:`ImagesPipeline`, the URLs in the
   ...
   pyrake scheduler and downloader (which means the scheduler and downloader
...
``images`` fields::
    ...
    class MyItem(pyrake.Item):

        # ... other item fields ...
        image_urls = pyrake.Field()
        images = pyrake.Field()
...
:setting:`ITEM_PIPELINES` setting::
    ...
    ITEM_PIPELINES = {'pyrake.contrib.pipeline.images.ImagesPipeline': 1}
...
.. module:: pyrake.contrib.pipeline.images
      ...
      image URL::
         ...
         def get_media_requests(self, item, info):
             for image_url in item['image_urls']:
                 yield pyrake.Request(image_url)
...
above::
    ...
    import pyrake
    from pyrake.contrib.pipeline.images import ImagesPipeline
    from pyrake.exceptions import DropItem
