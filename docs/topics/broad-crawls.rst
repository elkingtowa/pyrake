...
pyrake defaults are optimized for crawling specific sites. These sites are
often handled by a single pyrake spider, although this is not necessary or
...
As said above, pyrake default settings are optimized for focused crawls, not
broad crawls. However, due to its asynchronous architecture, pyrake is very
well suited for performing fast broad crawls. This page summarize some things
you need to keep in mind when using pyrake for doing broad crawls, along with
concrete suggestions of pyrake settings to tune in order to achieve an
...
The default global concurrency limit in pyrake is not suitable for crawling
...
trials and identifying at what concurrency your pyrake process gets CPU
...
get and any errors found. These stats are reported by pyrake when using the
...
pyrake handles (1) automatically; to handle (2) enable
