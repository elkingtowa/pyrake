...
The downloader middleware is a framework of hooks into pyrake's
request/response processing.  It's a light, low-level system for globally
altering pyrake's requests and responses.
...
:setting:`DOWNLOADER_MIDDLEWARES_BASE` setting defined in pyrake (and not meant to
...
as its value.  For example, if you want to disable the user-agent middleware::
    ...
    DOWNLOADER_MIDDLEWARES = {
        'myproject.middlewares.CustomDownloaderMiddleware': 543,
        'pyrake.contrib.downloadermiddleware.useragent.UserAgentMiddleware': None,
    }
...
.. module:: pyrake.contrib.downloadermiddleware
...
.. class:: DownloaderMiddleware
   ...
   .. method:: process_request(request, spider)
      ...
      :class:`~pyrake.http.Response` object, return a :class:`~pyrake.http.Request`
      object, or raise :exc:`~pyrake.exceptions.IgnoreRequest`.

      If it returns ``None``, pyrake will continue processing this request, executing all
      other middlewares until, finally, the appropriate downloader handler is called
      the request performed (and its response downloaded).

      If it returns a :class:`~pyrake.http.Response` object, pyrake won't bother
      ...
      If it returns a :class:`~pyrake.http.Request` object, pyrake will stop calling
      ...
      If it raises an :exc:`~pyrake.exceptions.IgnoreRequest` exception, the
   ...
   .. method:: process_response(request, response, spider)
      ...
      :meth:`process_response` should either: return a :class:`~pyrake.http.Response`
      object, return a :class:`~pyrake.http.Request` object or
      raise a :exc:`~pyrake.exceptions.IgnoreRequest` exception.

      If it returns a :class:`~pyrake.http.Response` (it could be the same given
      response, or a brand-new one), that response will continue to be processed
      with the :meth:`process_response` of the next middleware in the chain.

      If it returns a :class:`~pyrake.http.Request` object, the middleware chain is
      ...
      If it raises an :exc:`~pyrake.exceptions.IgnoreRequest` exception, the errback
      ...
      :type request: is a :class:`~pyrake.http.Request` object
   ...
   .. method:: process_exception(request, exception, spider)
      ...
      pyrake calls :meth:`process_exception` when a download handler
      or a :meth:`process_request` (from a downloader middleware) raises an
      exception (including an :exc:`~pyrake.exceptions.IgnoreRequest` exception)

      :meth:`process_exception` should return: either ``None``,
      a :class:`~pyrake.http.Response` object, or a :class:`~pyrake.http.Request` object.

      If it returns ``None``, pyrake will continue processing this exception,
      ...
      If it returns a :class:`~pyrake.http.Response` object, the :meth:`process_response`
      method chain of installed middleware is started, and pyrake won't bother calling
      any other :meth:`process_exception` methods of middleware.

      If it returns a :class:`~pyrake.http.Request` object, the returned request is
...
pyrake. For information on how to use them and how to write your own downloader
...
.. module:: pyrake.contrib.downloadermiddleware.cookies
...
For example::
    ...
    for i, url in enumerate(urls):
        yield pyrake.Request("http://www.example.com", meta={'cookiejar': i},
...
passing it along on subsequent requests. For example::
    ...
    def parse_page(self, response):
        # do some processing
        return pyrake.Request("http://www.example.com/otherpage",
...
If enabled, pyrake will log all cookies sent in requests (ie. ``Cookie``
...
.. module:: pyrake.contrib.downloadermiddleware.defaultheaders
...
.. module:: pyrake.contrib.downloadermiddleware.downloadtimeout
...
.. module:: pyrake.contrib.downloadermiddleware.httpauth
...
.. module:: pyrake.contrib.downloadermiddleware.httpcache
...
.. class:: HttpCacheMiddleware
    ...
    pyrake ships with two HTTP cache storage backends:
    ...
    pyrake ships with two HTTP cache policies:
...
* :setting:`HTTPCACHE_POLICY` to ``pyrake.contrib.httpcache.DummyPolicy``
...
* :setting:`HTTPCACHE_POLICY` to ``pyrake.contrib.httpcache.RFC2616Policy``
...
* :setting:`HTTPCACHE_STORAGE` to ``pyrake.contrib.httpcache.FilesystemCacheStorage``
...
``pyrake.utils.request.fingerprint``), and one level of subdirectories is
...
* :setting:`HTTPCACHE_STORAGE` to ``pyrake.contrib.httpcache.DbmCacheStorage``
...
the pyrake shell in parallel for the same spider.

In order to use this storage backend:

* set :setting:`HTTPCACHE_STORAGE` to ``pyrake.contrib.httpcache.LeveldbCacheStorage``
...
Default: ``'pyrake.contrib.httpcache.FilesystemCacheStorage'``
...
Default: ``'pyrake.contrib.httpcache.DummyPolicy'``
...
.. module:: pyrake.contrib.downloadermiddleware.httpcompression
...
.. module:: pyrake.contrib.downloadermiddleware.chunked
...
.. module:: pyrake.contrib.downloadermiddleware.httpproxy
...
.. class:: HttpProxyMiddleware
   ...
   ``proxy`` meta value to :class:`~pyrake.http.Request` objects.
...
.. module:: pyrake.contrib.downloadermiddleware.redirect
...
in the ``redirect_urls`` :attr:`Request.meta <pyrake.http.Request.meta>` key.
...
If :attr:`Request.meta <pyrake.http.Request.meta>` has ``dont_redirect``
...
.. module:: pyrake.contrib.downloadermiddleware.retry
...
If :attr:`Request.meta <pyrake.http.Request.meta>` has ``dont_retry`` key
...
.. module:: pyrake.contrib.downloadermiddleware.robotstxt
...
.. class:: RobotsTxtMiddleware
    ...
    To make sure pyrake respects robots.txt make sure the middleware is enabled
    and the :setting:`ROBOTSTXT_OBEY` setting is enabled.

    .. warning:: Keep in mind that, if you crawl using multiple concurrent
       requests per domain, pyrake could still  download some forbidden pages
...
.. module:: pyrake.contrib.downloadermiddleware.stats
...
.. module:: pyrake.contrib.downloadermiddleware.useragent
...
.. module:: pyrake.contrib.downloadermiddleware.ajaxcrawl
...
.. class:: AjaxCrawlMiddleware
   ...
   .. note::
       ...
       pyrake finds 'AJAX crawlable' pages for URLs like
