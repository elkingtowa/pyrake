...
pyrake uses :class:`Request` and :class:`Response` objects for crawling web
sites.
...
.. class:: Request(url[, callback, method='GET', headers, body, cookies, meta, encoding='utf-8', priority=0, dont_filter=False, errback])
    ...
    :param callback: the function that will be called with the response of this
       ...
       :meth:`~pyrake.spider.Spider.parse` method will be used.
    ...
    :param cookies: the request cookies. These can be sent in two forms.
        ...
        pyrake to do so by setting the ``dont_merge_cookies`` key to True in the
        :attr:`Request.meta`.
    ...
    .. attribute:: Request.meta
        ...
        empty for new Requests, and is usually  populated by different pyrake
        components (extensions, middlewares, etc). So the data contained in this
        dict depends on the extensions you have enabled.

        See :ref:`topics-request-meta` for a list of special meta keys
        recognized by pyrake.
...
Example::
    ...
    def parse_page1(self, response):
        return pyrake.Request("http://www.example.com/some_page.html",
...
different fields from different pages::
    ...
    def parse_page1(self, response):
        ...
        request = pyrake.Request("http://www.example.com/some_page.html",
                                 callback=self.parse_page2)
...
are some special keys recognized by pyrake and its built-in extensions.
...
method for this job. Here's an example spider which uses it::
    ...
    class LoginSpider(pyrake.Spider):
        name = 'example.com'
        start_urls = ['http://www.example.com/users/login.php']

        def parse(self, response):
            return pyrake.FormRequest.from_response(
                response,
...
.. class:: Response(url, [status=200, headers, body, flags])
    ...
    .. attribute:: Response.request
        ...
        assigned in the pyrake engine, after the response and the request have passed
...
.. class:: TextResponse(url, [encoding[, ...]])
    ...
    .. attribute:: TextResponse.selector
        ...
        A :class:`~pyrake.selector.Selector` instance using the response as
