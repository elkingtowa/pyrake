...
pages (:class:`pyrake.http.Response` objects) which will be eventually
followed.

There is ``pyrake.contrib.linkextractors import LinkExtractor`` available
in pyrake, but you can create your own custom Link Extractors to suit your
needs by implementing a simple interface.

The only public method that every link extractor has is ``extract_links``,
which receives a :class:`~pyrake.http.Response` object and returns a list
of :class:`pyrake.link.Link` objects. Link extractors are meant to be
instantiated once and their ``extract_links`` method called several times
with different responses to extract links to follow.

Link extractors are used in the :class:`~pyrake.contrib.spiders.CrawlSpider`
class (available in pyrake), through a set of rules, but you can also use it in
your spiders, even if you don't subclass from
:class:`~pyrake.contrib.spiders.CrawlSpider`, as its purpose is very simple: to
extract links.
...
.. module:: pyrake.contrib.linkextractors
   :synopsis: Link extractors classes

Link extractors classes bundled with pyrake are provided in the
:mod:`pyrake.contrib.linkextractors` module.
...
There used to be other link extractor classes in previous pyrake versions,
but they are deprecated now.

LxmlLinkExtractor
-----------------

.. module:: pyrake.contrib.linkextractors.lxmlhtml
...
.. class:: LxmlLinkExtractor(allow=(), deny=(), allow_domains=(), deny_domains=(), deny_extensions=None, restrict_xpaths=(), tags=('a', 'area'), attrs=('href',), canonicalize=True, unique=True, process_value=None)
    ...
    :param deny_extensions: a single value or list of strings containing
        ...
        ``IGNORED_EXTENSIONS`` list defined in the `pyrake.linkextractor`_
        module.
    ...
    :param canonicalize: canonicalize each extracted url (using
        pyrake.utils.url.canonicalize_url). Defaults to ``True``.
...
.. _pyrake.linkextractor: https://github.com/pyrake/pyrake/blob/master/pyrake/linkextractor.py
