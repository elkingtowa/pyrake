...
In pyrake, objects such as Requests, Responses and Items have a finite
...
As these pyrake objects have a (rather long) lifetime, there is always the risk
of accumulating them in memory without releasing them properly and thus causing
what is known as a "memory leak".

To help debugging memory leaks, pyrake provides a built-in mechanism for
...
pyrake developer passes objects referenced in Requests (for example, using the
:attr:`~pyrake.http.Request.meta` attribute or the request callback function)
...
in pyrake projects, and a quite difficult one to debug for newcomers.
...
``trackref`` is a module provided by pyrake to debug the most common cases of
...
alias to the :func:`~pyrake.utils.trackref.print_live_refs` function::
...
each class using the :func:`~pyrake.utils.trackref.get_oldest` function like
...
* ``pyrake.http.Request``
* ``pyrake.http.Response``
* ``pyrake.item.Item``
* ``pyrake.selector.Selector``
* ``pyrake.spider.Spider``
...
compared to Requests. So let's check the oldest response::
    ...
    >>> from pyrake.utils.trackref import get_oldest
...
can use the :func:`iter_all` function::
    ...
    >>> from pyrake.utils.trackref import iter_all
...
.. module:: pyrake.utils.trackref
   :synopsis: Track references of live objects

pyrake.utils.trackref module
----------------------------

Here are the functions available in the :mod:`~pyrake.utils.trackref` module.
...
the heap using Guppy::
    ...
    Partition of a set of 297033 objects. Total size = 52587824 bytes.
     Index  Count   %     Size   % Cumulative  % Type
         ...
         9   1209   0   456936   1  49872920  95 pyrake.http.headers.Headers
...
Sometimes, you may notice that the memory usage of your pyrake process will
only increase, but never decrease. Unfortunately, this could happen even
though neither pyrake nor your project are leaking memory. This is due to a
...
This problem will be fixed in future pyrake releases, where we plan to adopt a
