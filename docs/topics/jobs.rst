...
pyrake supports this functionality out of the box by providing the following
facilities:
...
To start a spider with persistence supported enabled, run it like this::
    ...
    pyrake crawl somespider -s JOBDIR=crawls/somespider-1
...
There are a few things to keep in mind if you want to be able to use the pyrake
persistence support:
...
So, for example, this won't work::
    ...
    def some_callback(self, response):
        somearg = 'test'
        return pyrake.Request('http://www.example.com', callback=lambda r: self.other_callback(r, somearg))
...
But this will::
    ...
    def some_callback(self, response):
        somearg = 'test'
        return pyrake.Request('http://www.example.com', meta={'somearg': somearg})
