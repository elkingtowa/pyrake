...
This document describes the architecture of pyrake and how its components
interact.

Overview
========

The following diagram shows an overview of the pyrake architecture with its
...
.. image:: _images/pyrake_architecture.png
   :width: 700
   :height: 494
   :alt: pyrake architecture
...
Spiders are custom classes written by pyrake users to parse responses and
...
a convenient mechanism for extending pyrake functionality by plugging custom
...
requests). They provide a convenient mechanism for extending pyrake
...
The data flow in pyrake is controlled by the execution engine, and goes like
this:
...
pyrake is written with `Twisted`_, a popular event-driven networking framework
