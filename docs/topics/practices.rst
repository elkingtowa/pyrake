...
This section documents common practices when using pyrake. These are things
that cover many topics and don't often fall into any other specific section.

.. _run-from-script:

Run pyrake from a script
========================

You can use the :ref:`API <topics-api>` to run pyrake from a script, instead of
the typical way of running pyrake via ``pyrake crawl``.

Remember that pyrake is built on top of the Twisted
...
::
    ...
    from pyrake.crawler import Crawler
    from pyrake import log, signals
...
By default, pyrake runs a single spider per process when you run ``pyrake
crawl``. However, pyrake supports running multiple spiders per process using
...
pyrake doesn't provide any built-in facility for running crawls in a distribute
...
many pyraked instances and distribute spider runs among those.
...
Then you fire a spider run on 3 different pyraked servers. The spider would
receive a (spider) argument ``part`` with the number of the partition to
crawl::

    curl http://pyrake1.mycompany.com:6800/schedule.json -d project=myproject -d spider=spider1 -d part=1
    curl http://pyrake2.mycompany.com:6800/schedule.json -d project=myproject -d spider=spider1 -d part=2
    curl http://pyrake3.mycompany.com:6800/schedule.json -d project=myproject -d spider=spider1 -d part=3
...
.. _commercial support: http://pyrake.org/support/
...
::
    ...
    from pyrake.item import DictItem, Field
