...
from pyrake.core.downloader import webclient as client
from pyrake.http import Request, Headers
...
def getPage(url, contextFactory=None, *args, **kwargs):
    ...
    def _clientfactory(*args, **kwargs):
        ...
        f = client.pyrakeHTTPClientFactory(Request(*args, **kwargs), timeout=timeout)
...
class ParseUrlTestCase(unittest.TestCase):
    ...
    def _parse(self, url):
        f = client.pyrakeHTTPClientFactory(Request(url))
    ...
    ("http://pyraketest.org/foo ",    ('http', 'pyraketest.org', 'pyraketest.org', 80, '/foo')),
...
class pyrakeHTTPPageGetterTests(unittest.TestCase):

    def test_earlyHeaders(self):
        # basic test stolen from twisted HTTPageGetter
        factory = client.pyrakeHTTPClientFactory(Request(
            url='http://foo/bar',
            body="some data",
            headers={
                'Host': 'example.net',
        ...
        factory = client.pyrakeHTTPClientFactory(Request('http://foo/bar'))
        self._test(factory,
            "GET /bar HTTP/1.0\r\n"
            "Host: foo\r\n"
            "\r\n")
    ...
    def test_non_standard_line_endings(self):
        # regression test for: http://dev.pyrake.org/ticket/258
        factory = client.pyrakeHTTPClientFactory(Request(
            url='http://foo/bar'))
        protocol = client.pyrakeHTTPPageGetter()
        protocol.factory = factory
...
class WebClientTestCase(unittest.TestCase):
    ...
    def testFactoryInfo(self):
        ...
        factory = client.pyrakeHTTPClientFactory(Request(url))
