...
class ProjectTest(unittest.TestCase):
    ...
    def call(self, *new_args, **kwargs):
        with tempfile.TemporaryFile() as out:
            args = (sys.executable, '-m', 'pyrake.cmdline') + new_args
...
class StartprojectTest(ProjectTest):
    ...
    def test_startproject(self):
        ...
        assert exists(join(self.proj_path, 'pyrake.cfg'))
...
class CommandTest(ProjectTest):
    ...
    def setUp(self):
        ...
        self.env['pyrake_SETTINGS_MODULE'] = '%s.settings' % self.project_name
...
""" % self.project_name)
    ...
    def test_pipelines(self):
        p = self.proc('parse', '--spider', self.spider_name, '--pipelines',
                '-c', 'parse', 'http://scrapinghub.com')
        log = p.stderr.read()
        self.assert_("[pyrake] INFO: It Works!" in log, log)
