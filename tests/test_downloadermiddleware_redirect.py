...
from pyrake.contrib.downloadermiddleware.redirect import RedirectMiddleware, MetaRefreshMiddleware
from pyrake.spider import Spider
from pyrake.exceptions import IgnoreRequest
from pyrake.http import Request, Response, HtmlResponse
...
class RedirectMiddlewareTest(unittest.TestCase):
    ...
    def test_max_redirect_times(self):
        ...
        req = Request('http://pyraketest.org/302')
        rsp = Response('http://pyraketest.org/302', headers={'Location': '/redirected'}, status=302)
    ...
    def test_ttl(self):
        ...
        req = Request('http://pyraketest.org/302', meta={'redirect_ttl': 1})
        rsp = Response('http://www.pyraketest.org/302', headers={'Location': '/redirected'}, status=302)
    ...
    def test_redirect_urls(self):
        req1 = Request('http://pyraketest.org/first')
        rsp1 = Response('http://pyraketest.org/first', headers={'Location': '/redirected'}, status=302)
        req2 = self.mw.process_response(req1, rsp1, self.spider)
        rsp2 = Response('http://pyraketest.org/redirected', headers={'Location': '/redirected2'}, status=302)
        req3 = self.mw.process_response(req2, rsp2, self.spider)

        self.assertEqual(req2.url, 'http://pyraketest.org/redirected')
        self.assertEqual(req2.meta['redirect_urls'], ['http://pyraketest.org/first'])
        self.assertEqual(req3.url, 'http://pyraketest.org/redirected2')
        self.assertEqual(req3.meta['redirect_urls'], ['http://pyraketest.org/first', 'http://pyraketest.org/redirected'])
...
class MetaRefreshMiddlewareTest(unittest.TestCase):
    ...
    def test_max_redirect_times(self):
        self.mw.max_redirect_times = 1
        req = Request('http://pyraketest.org/max')
        rsp = HtmlResponse(req.url, body=self._body())
